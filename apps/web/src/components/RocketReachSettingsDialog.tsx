...
interface RocketReachSettingsDialogProps {
  open: boolean;
  onClose: () => void;
}
...
/**
 * Settings dialog for the optional RocketReach prospect-research integration.
 * The raw API key never leaves the server, so this dialog only ever shows a
 * masked hint; the input is write-only (type a new key to replace it).
 */
export function RocketReachSettingsDialog({ open, onClose }: RocketReachSettingsDialogProps) {
  ...
  return (
    <div className="modal-backdrop" role="presentation" onClick={onClose}>
      <div
        className="modal-dialog modal-info rocketreach-settings"
        role="dialog"
        aria-modal="true"
        aria-labelledby="rocketreach-settings-title"
        ref={dialogRef}
        tabIndex={-1}
        onClick={(event) => event.stopPropagation()}
      >
        <h2 id="rocketreach-settings-title">RocketReach integration</h2>
        <p>
          Optional prospect-research integration. Off by default. When enabled, connected agents get
          RocketReach tools for guided intake, people search, and (paid) contact lookups. Your API
          key is stored on your machine (never in notes) and is never shown again once saved.
        </p>
